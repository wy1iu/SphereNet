...
Baseline_CNN.build(images, n_class, is_training)

fit_loss = loss2(Baseline_CNN.score, labels, n_class, 'c_entropy') 
loss_op = fit_loss
...
acc_op = tf.reduce_mean(tf.to_float(tf.equal(labels, tf.to_int32(Baseline_CNN.pred))))
