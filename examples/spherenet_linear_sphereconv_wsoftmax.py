...
class SphereNet():
    ...
    def build(self, rgb, n_class, is_training):        
        ...
        for i in range(n_layer):
            feat = self._conv_layer(feat, ksize, n_out, is_training, name="conv1_"+str(i), bn=True, relu=True, pad='SAME', norm='linear', reg=False, orth=True)
        feat = self._max_pool(feat, 'pool1')

        #16X16
        n_out = 196
        for i in range(n_layer):
            feat = self._conv_layer(feat, ksize, n_out, is_training, name="conv2_"+str(i), bn=True, relu=True, pad='SAME', norm='linear', reg=False, orth=True)
        feat = self._max_pool(feat, 'pool2')

        #8X8
        n_out = 256
        for i in range(n_layer):
            feat = self._conv_layer(feat, ksize, n_out, is_training, name="conv3_"+str(i), bn=True, relu=True, pad='SAME', norm='linear', reg=False, orth=True)
        feat = self._max_pool(feat, 'pool3')      

        self.fc6 = self._conv_layer(feat, 4, 256, is_training, "fc6", bn=True, relu=False, pad='VALID', norm='linear', reg=False, orth=True)        

        self.score = self._conv_layer(self.fc6, 1, n_class, is_training, "score", bn=False, relu=False, pad='VALID', norm='none', reg=True, orth=False, w_norm='cosine')
